...
func (sk *StakingPlugin) GetAdjustmentStakingDelegate(blockHash common.Hash, blockNumber uint64) (staking.AdjustmentStakingDelegateQueue, error) {
	numStr := strconv.FormatUint(blockNumber, 10)
	log.Debug("wow,GetAdjustmentStakingDelegate query number:", "num string", numStr)

	data, err := STAKING_DB.HistoryDB.Get([]byte(AdjustmentName + numStr))
	if nil != err {
		return nil, err
	}
	var adjustmentStakingDelegateQueue staking.AdjustmentStakingDelegateQueue
	err = rlp.DecodeBytes(data, &adjustmentStakingDelegateQueue)
	if nil != err {
		return nil, err
	}
	snq := make(staking.AdjustmentStakingDelegateQueue, len(adjustmentStakingDelegateQueue))
	for i, v := range adjustmentStakingDelegateQueue {
		snq[i] = &staking.AdjustmentStakingDelegateData{
			OptType:         v.OptType,
			NodeId:          v.NodeId,
			StakingBlockNum: v.StakingBlockNum,
			Addr:            v.Addr,
